...
console.log('governor :>> ', governor);
  ...
  const EditGovernor = async (values) => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        console.log("Governor Edited successfully", data);
  ...
  return (
    <>
      <h1 className="text-3xl font-bold">Edit Governor</h1>
      {governor?  <div className="flex flex-row px-5 py-10">
        <Formik
          initialValues={{
            fullName: governor?.fullName,
                  ...
                  <div className="flex flex-row flex-wrap items-center">
                    ...
                    <div>
                      <TextInput
                        ...
                        onBlur={handleBlur}
                        required
                        color={errors.year && touched.year ? "failure" : ""}
                        helperText={
...
export default EditGovernorHistory;
